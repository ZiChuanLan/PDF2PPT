...
def test_notebooklm_footer_detector_accepts_longer_mineru_footer_text() -> None:
    page_w_pt = 720.0
    page_h_pt = 405.0
        ...
        generator._is_notebooklm_footer_text_element(
            {
                ...
                "bbox_pt": [592.0, 360.0, 704.0, 384.0],
            },
            page_w_pt=page_w_pt,
            page_h_pt=page_h_pt,
        )
...
def test_generate_pptx_mineru_text_page_fullpage_keeps_images_in_background(
    monkeypatch, tmp_path: Path
) -> None:
    cleared_region_calls: list[list[list[float]]] = []
    ...
    def _fake_erase_regions_in_render_image(
        ...
        *,
        ...
        out_path.parent.mkdir(parents=True, exist_ok=True)
        Image.open(render_path).save(out_path)
        return out_path

    def _fake_clear_regions_for_transparent_crops(
        *,
        ...
        page_height_pt: float,
        dpi: int,
        clear_expand_min_pt: float,
        clear_expand_max_pt: float,
        clear_expand_ratio: float,
    ) -> Path:
        del pix, page_height_pt, dpi, clear_expand_min_pt, clear_expand_max_pt, clear_expand_ratio
        cleared_region_calls.append([list(bb) for bb in regions_pt])
        out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    source_pdf = tmp_path / "mineru-fullpage.pdf"
    source_pdf.write_bytes(b"%PDF-1.4\n%stub\n")
    image_path = tmp_path / "figure.png"
    Image.new("RGB", (120, 80), color=(255, 120, 60)).save(image_path)
            ...
            "pages": [
                {
                    ...
                    "elements": [
                        ...
                        },
                        {
                            ...
                            "bbox_pt": [300.0, 140.0, 540.0, 300.0],
                        },
                    ],
                }
            ],
        },
        tmp_path / "mineru-fullpage.pptx",
        artifacts_dir=tmp_path / "mineru-fullpage-artifacts",
        export_final_preview_images=False,
