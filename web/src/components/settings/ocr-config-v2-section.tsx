...
const LAYOUT_MODEL_OPTIONS = Object.values(LAYOUT_MODELS).map((m) => ({
  id: m.modelId as OcrConfigV2["layout"]["model"],
  label: m.displayName,
  sizeMb: m.sizeMb,
}))
...
type OcrConfigV2SectionProps = {
  settings: Settings
  onSettingsChange: (updates: Partial<Settings>) => void
}

export function OcrConfigV2Section({ settings, onSettingsChange }: OcrConfigV2SectionProps) {
  // Convert current settings to V2 config
  const config = React.useMemo(() => migrateSettingsToOcrConfigV2(settings), [settings])
  ...
  const updateConfig = (updates: Partial<OcrConfigV2>) => {
    const newConfig = { ...config, ...updates }
    const settingsUpdates = applyOcrConfigV2ToSettings(newConfig, settings)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-base font-semibold">OCR 配置</h3>
      ...
      <div className="space-y-4 rounded-lg border bg-muted/30 p-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm font-medium">第一层：版面检测（可选）</div>
        ...
        {config.layout.enabled && (
          <div className="space-y-4 pt-2">
              ...
              <Select
                ...
                onChange={(e) =>
                  updateConfig({
                    layout: {
                      ...config.layout,
                      model: e.target.value as OcrConfigV2["layout"]["model"],
                    },
                  })
                }
            ...
            <div className="rounded-lg bg-blue-50 dark:bg-blue-950/30 p-3 text-xs text-blue-900 dark:text-blue-100">
              <strong>说明：</strong>版面检测会先将页面切分为多个区域（文本块、图像块、表格等），
              然后对每个区域分别进行识别。这样可以提高识别精度，特别是对于复杂文档。
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4 rounded-lg border bg-muted/30 p-4">
        <div>
          <div className="text-sm font-medium">第二层：文字识别（必选）</div>
          <p className="text-xs text-muted-foreground">
        ...
        <div className="space-y-3">
          {(["paddleocr", "tesseract", "aiocr"] as TextRecognitionProvider[]).map(
            (provider) => (
              <label
                key={provider}
      ...
      <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 p-4 text-sm text-amber-900 dark:text-amber-100">
        <div className="font-medium mb-2">架构说明</div>
        <ul className="space-y-1 text-xs list-disc list-inside">
          <li>
            <strong>版面检测</strong>是独立的第一层，可以配合任何识别引擎使用
          </li>
          <li>
            <strong>PaddleOCR</strong> 内部总是使用版面检测（无法关闭）
          </li>
          <li>
            <strong>Tesseract</strong> 可选使用版面检测（先切块，再识别每个块）
          </li>
          <li>
            <strong>AI OCR</strong> 可选使用版面检测（layout_block 模式）
          </li>
          <li>
